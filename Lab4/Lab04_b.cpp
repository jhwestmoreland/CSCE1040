...
#include <string>
/********************************************************************
 **        NAME:  Jared Westmoreland         USERID:jhw0094        **
...
{ // ***MAIN***
  ...
  FILE *in_stream;
//  in_stream = fopen("twentywords.txt", "r");             // identify the input file
  char word[MAX_WORDS];      // temp holds words as read char-by-char

  int i;                     // general purpose iterator
  int str_len;               // actual string length of input words
  int wordNum;               // word number being actively worked
  int asciiVal;              // ascii value of input characters
  int asciiTot;              // total ascii val of all letter in word
  int t_count;               // count of "t" characters in a word
  ...
  printf("|  NAME:  Jared Westmoreland      EUID:jhw0094       |\n");
  printf("|  EMAIL: JaredWestmoreland@my.unt.edu               |\n");
...
//  if (in_stream == NULL)
  //{          // ***IF in_stream.fail***
    // printf("Unable to open input file: twentywords.txt \n\n");
    // return 1;
  //}          // ***IF in_stream.fail***
 ...
 *     - clearing to the next line                     *
  ...
  while (in_stream != NULL && wordNum < MAX_WORDS)
    {     // ***WHILE not eof***
    ...
    for (i=0; i<=str_len-1 ;++i)
      {
      // FOR i to str_len-1
      word[i] = getchar();      //get the next character
